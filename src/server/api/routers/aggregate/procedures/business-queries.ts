...
export const getBusinessById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    const buildingQuery = await ctx.db
      ...
      .where(
        sql`${lungriAggregateBuilding.businesses} @> jsonb_build_array(jsonb_build_object('id', ${input.id}))`,
      )
      .limit(1);
...
export const getBusinessCrops = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const building = await ctx.db
      ...
      .from(lungriAggregateBuilding)
      .where(
        sql`${lungriAggregateBuilding.businesses} @> jsonb_build_array(jsonb_build_object('id', ${input.businessId}))`,
      )
      .limit(1);
