...
export const getHouseholdById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    const buildingQuery = await ctx.db
      ...
      .where(
        sql`${lungriAggregateBuilding.households} @> jsonb_build_array(jsonb_build_object('id', ${input.id}))`,
      )
      .limit(1);
...
export const getHouseholdsByBuildingId = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const building = await ctx.db
      .select()
      .from(lungriAggregateBuilding)
      .where(eq(lungriAggregateBuilding.id, input.buildingId))
      .limit(1);
...
export const getHouseholdMembers = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const building = await ctx.db
      ...
      .from(lungriAggregateBuilding)
      .where(
        sql`${lungriAggregateBuilding.households} @> jsonb_build_array(jsonb_build_object('id', ${input.householdId}))`,
      )
      .limit(1);
