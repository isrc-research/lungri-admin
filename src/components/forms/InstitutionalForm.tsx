...
const InstitutionalForm = ({ areaId }: InstitutionalFormProps) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const startY = await setupPDFHeader(doc, {
        ...
        formTitle: "Institutional Survey Form",
        formSubtitle: "Physical Infrastructure Assessment",
      });

      const tableColumns = [
        { header: "SN", dataKey: "sn" },
        { header: "Name of Institution", dataKey: "name_of_institution" },
      ...
      doc.save(`institutional_form_${areaDetails.code}.pdf`);
    } catch (error) {
  ...
  return (
    <PrintButton
      icon={Printer}
      label={`Institutional Form (${tokenCount})`}
      onPrint={generatePDF}
